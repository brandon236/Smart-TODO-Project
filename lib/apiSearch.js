...
const apiSearch = (db, input, userName) => {
  let category_id = 5;
  ...
  "vanilla ice cream", "chocolate chip cookies", "pizza", "kebab", "kebabs", "paint"];
  ...
  return requestPromise(website.movie)
  .then(function (data) {
    if (data.Response === 'True') {
      ...
      .then(function (data) {
        ...
        .then(function (data) {
          if (data.length > 0 && input.toLowerCase() !== "take a nap") {
            return 4;
         }
  ...
  .then(function (category_id) {
    ...
    duplicateCheck (db, input, userName)
    .then((data) => {
      return data;
    })
    .then((data) => {
      ...
      query = `INSERT INTO list (task_description, user_id, category_id) VALUES ($1, (SELECT id FROM users WHERE username='${userName}'), ${category_id});`;
